...
## Remediation status update (27 Jun 2026, hardening 4/N — job-smcsec-4)

Done this pass (PR open for orchestrator review, not yet merged): **F2 — per-device helper-key revocation**, the last open item on the §6 priority list before the Low-rated F6/F7/F8 (all already closed).

- **F2 — per-device pairing keys + individual revocation list.** Before this pass a user had a single pairing key `{email, version}`; the only revocation was bumping `helper_key_version`, which kicked *every* one of that user's devices at once. Now each registered device gets a **server-issued, unguessable device id** (`hd_…`, no PII) baked into its signed key (`d` claim) and recorded in a new **`helper_devices`** table (`device_id` PK, `user_email`, `label`, `created_at`, `last_seen_at`, `revoked_at`; indexed by user). The engine's `validate-helper-key` callback now applies a per-device layer **on top of** the existing HMAC + version checks: a key whose device row is missing or `revoked_at` is set is rejected, and `last_seen_at` is touched (fire-and-forget) on a good connection. Revoking one device leaves the others working.
- **Decision logic isolated for real test coverage.** The accept/reject decision lives in a pure, Next-free module **`app/lib/helper-devices.js`** (`newHelperDeviceId`, `helperDeviceDecision`) so it is unit-tested offline with genuine coverage (not a source-grep): active→accept, revoked→reject, unknown→reject, cross-user row→reject, legacy-key→accept, null/garbage→fail-closed.
- **Profile UX.** `/profile` replaces the single-key card with a **paired-devices** list (label, added date, last seen, per-device **Revoke**), a **Pair-a-device** action that mints a key shown **once**, and a **Revoke-all** panic button (the old `helper_key_version` bump, now also flagging every device row revoked).
- **Migration bridge (no breakage).** A pre-F2 key (no `d` claim) still validates on the coarse checks alone (`helperDeviceDecision` returns `{ ok:true, legacy:true }`), so a currently-paired helper keeps working until the user re-pairs; a future hard cutover can reject legacy keys once every device has re-paired. The Cloudflare worker and the Electron helper need **no code change** — the device id rides transparently inside the signed key the user already pastes.

Verified: `node --check` clean on all changed files; `next build` compiles (all routes incl. `/profile`, `/api/helper-key`, `/api/internal/validate-helper-key`); `node scripts/test-helper-devices.mjs` **46/0** (real decision-logic coverage + key-codec + route/migration/profile source checks); `npm run test:security` green across retention 20/0 + IDOR 57/0 + rate-limit 44/0 + pairing 53/0 + devices 46/0; `npm run test:bot` 31/0 (worker untouched). With F2 closed, every F-finding (F1–F8) and every Section-8 High/medium is now implemented and tested; the only outstanding security item is the **operator action** to rotate the GitHub credential embedded in the Mac git remote (outside this repo).

 ...
 *     owned-table access by the authenticated session email. The guard is matched
 *     tolerant of the variable name the route binds the payload to: routes use
 *     either `const session = await getSessionPayload()` or `const p = ...`, and
 *     scope with `user_email = ${session.email}`, `${p.email}`, a bare `${email}`,
 *     a join alias `m.user_email = ${...}`, or a `FROM meetings WHERE user_email`
 *     subquery for child tables. All of these contain the substring
 *     `user_email = ${...}`, which is the invariant we assert. (The earlier
 *     version pinned the literal `${session.email}` and produced false failures
 *     on the `p.`/`${email}` routes; that was the cause of the revert.)
...
// email. Tolerant of the variable the route binds (session.email, p.email, a
// bare email, a join alias m.user_email, or a meetings subquery) — all of these
// contain `user_email = ${...}`.
const OWNERSHIP_GUARD = /user_email\s*=\s*\$\{[^}]+\}/;
...
if (!dbUrl) {
  console.log('  SKIP DATABASE_URL not set — skipping live DB IDOR test (source audit above stands)');
} else {
  ...
  try {
    ...
    if (/permission denied|read-only|cannot execute (INSERT|UPDATE|DELETE)/i.test(e.message)) {
      console.log(`  SKIP Read-only DB — skipping live IDOR test (${e.message.split('\n')[0]})`);
    } else {

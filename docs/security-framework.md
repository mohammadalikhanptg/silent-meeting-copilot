...
F7 — Low — IDOR audit. **(audit complete; automated regression test added 26 Jun)**
The routes reviewed scope by session email, but not every route had been audited. Remediation: enumerate every data route and assert ownership scoping; add an automated test that a second user cannot read the first user's meetings, segments, reference docs, downloads, or flagged items. STATUS: a full route-by-route audit was completed — all ~20 data routes (meetings and every meeting child: prep, segments, segments/[segId], ref-docs, ref-docs/[docId], minutes, minutes-docx, action-points, interview-assessment, transcript; flagged-items and [itemId]/[itemId]/process; profile; profile-docs and [docId]) constrain owned-table access by the authenticated session email, directly or via a join/subquery to meetings.user_email; no unscoped route was found. An automated regression test (`scripts/test-idor.mjs`, `npm run test:idor`) asserts ownership scoping for every data route (source-level, runs anywhere — 57 assertions pass) plus a live cross-user DB test that builds user A's full data graph and verifies user B's exact route queries return 0 rows for meetings, segments, reference docs (CVs), flagged items, profile and profile docs. The live DB layer runs when DATABASE_URL is write-capable and skips cleanly otherwise (not yet executed against a writable DB in this environment).
...
Still open (next dedicated pass, best done after the operator's meeting test because they touch the live authentication path or secret rotation): H2 move the helper key and engine token out of the URL query string into a header/subprotocol; H3 drop the HELPER_SIGNING_SECRET internal fallback once INTERNAL_SHARED_SECRET is confirmed, add a signing-key id for staged rotation; H4 bind the engine token to the active app session id with a revocation check and jti replay protection. Mediums remaining: role-scope broadcasts so helpers do not receive transcript messages; prompt-injection isolation for reference docs/transcripts in the coach prompt (the interview assessment already isolates them); a strict Content-Security-Policy; data retention and hard-delete; AI-provider data-processing confirmation; CI dependency and secret scanning. (The automated IDOR test is now done — see F7; a full route-by-route audit plus `scripts/test-idor.mjs`.) The gate stands: no real third-party or candidate data until H2-H4 and the broadcast-scoping, prompt-injection and retention mediums are implemented and tested.

...
3. Security hardening, gated before any real third-party/candidate data: tokens out of the URL (needs helper rebuild), drop the signing-secret fallback with a key id for rotation, bind the engine token to the session with revocation/replay protection, strict CSP, retention/hard-delete, ~~IDOR test~~ (done — see below), AI-provider data agreement, CI scanning, rotate the git credential. (Mostly engine-side: deploy via wrangler from the Mac, so not completable from a web/Windows-only session.)
4. Phase 2: speaker-labelled note-taker paid option; softphone auto start/stop; per-vertical scaling.

## Progress — 26 Jun 2026
- Automated IDOR regression test (security finding F7) — done. A full route-by-route audit confirmed every one of the ~20 data routes (meetings + all meeting children: prep, segments, segments/[segId], ref-docs, ref-docs/[docId], minutes, minutes-docx, action-points, interview-assessment, transcript; flagged-items + [itemId] + [itemId]/process; profile; profile-docs + [docId]) scopes owned-table access by the authenticated session email, directly or via a join/subquery to meetings.user_email; no unscoped route exists. Added `scripts/test-idor.mjs` (+ `npm run test:idor`): a source-level audit (57 assertions, runs anywhere — all pass) plus a live cross-user DB test (builds user A's data graph, asserts user B's exact route queries return 0 rows across meetings, transcript segments, reference docs/CVs, flagged items, profile and profile docs) that runs when DATABASE_URL is write-capable and skips cleanly otherwise. Pure test/docs change — no app or engine code touched, no deploy risk. Docs updated: security-framework.md F7 + section-8 medium list. The live DB layer has not yet been executed against a writable DB in this environment; remaining security mediums are unchanged (CSP, retention/hard-delete, AI-provider agreement, CI scanning) and the H2–H4 engine items still need the Mac/wrangler.

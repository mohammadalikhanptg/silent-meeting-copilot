...
## Progress — 27 Jun 2026 (later still) — security hardening 4/N (job-smcsec-4)

PR opened (not merged; orchestrator reviews/merges/deploys):
- **F2 per-device helper-key revocation — DONE.** The last item on the §6 priority list (F6/F7/F8 already closed). Each helper device now gets a server-issued, PII-free device id (`hd_…`) baked into its signed pairing key (`d` claim) and tracked in a new `helper_devices` table (`device_id` PK, `user_email`, `label`, `created_at`, `last_seen_at`, `revoked_at`). The engine's `validate-helper-key` callback rejects keys whose device row is missing or revoked, **on top of** the existing HMAC + `helper_key_version` checks — so one leaked/retired device is revoked individually without kicking the others (the old version bump remains as a "revoke all" panic button).
- **Profile UX:** `/profile` now shows a **paired-devices** list with per-device **Revoke**, a **Pair-a-device** action (key shown once), and **Revoke-all**.
- **No breakage:** legacy pre-F2 keys (no `d`) still validate on the coarse checks (migration bridge); the Cloudflare worker and Electron helper need **no change** (the device id rides inside the key the user already pastes). Decision logic isolated in a pure `app/lib/helper-devices.js` for real offline test coverage.
- Verified: `next build` compiles all routes; `node scripts/test-helper-devices.mjs` **46/0**; `test:security` green (retention 20 + IDOR 57 + rate-limit 44 + pairing 53 + devices 46, 0 failed); `test:bot` 31/0. With F2 closed, **every F1–F8 finding and every Section-8 High/medium is implemented and tested**; the sole outstanding security item is the operator action to rotate the git-embedded GitHub credential (outside this repo).

...
- Next, in priority order: F4 data retention + hard-delete + at-rest (gate before any real third-party/candidate data), then F2 per-device helper-key revocation, F3 drop the signing-secret fallback + add a rotation key id, F6 strict CSP + session lifetime, the F5 error-leakage sub-item, and F1 rate-limiting (criticals/CORS/headers already closed).

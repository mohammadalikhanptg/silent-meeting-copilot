...
const MAX_TOTAL_BYTES = 512 * 1024; // 512 KB total per user
...
export async function POST(request) {
  ...
  const [counts] = await sql`
    ...
    FROM profile_docs WHERE user_email = ${session.email}
  `;
  if (counts.file_count >= MAX_FILES_PER_USER) {
    return NextResponse.json(
      { error: `Maximum ${MAX_FILES_PER_USER} profile documents. Remove one before uploading another.` },
      { status: 422 }
    );
  }
  if (Number(counts.total_bytes) + contentBytes > MAX_TOTAL_BYTES) {
    return NextResponse.json(
      { error: 'Total profile document size limit (512 KB) reached. Remove a document to free space.' },
      { status: 422 }
    );
  }
  ...
  const [doc] = await sql`
    INSERT INTO profile_docs (user_email, filename, content_text)
    VALUES (${session.email}, ${safeName}, ${sanitized})

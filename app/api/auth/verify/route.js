...
export async function GET(req) {
  ...
  const fail = NextResponse.redirect(`${base}/login?e=invalid`);
  ...
  const row = rows[0];
  if (!row || row.consumed_at || new Date(row.expires_at) < new Date()) return fail;
  ...
  c.set(PRE_COOKIE, preCookieValue(row.email, stage), cookieOptions(PRE_MAXAGE));
  return res;
}
